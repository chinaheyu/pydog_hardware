...
PCA9685_STATUS PCA9685_SetPwmFrequency(uint16_t Frequency)
{
	...
	{
		PrescalerVal = (25000000 / (4096.0f * (float)Frequency)) - 1;
		Prescale = floor(PrescalerVal + 0.5f);
	}
...
PCA9685_STATUS PCA9685_SetServoAngles(float* Angles, uint8_t Channel, int num)
{
    uint8_t RegisterAddress;
    ...
    for(int i = 0; i < num; ++i)
    {
        ...
        Value = (Angles[i] - MIN_ANGLE) * ((float)SERVO_MAX - (float)SERVO_MIN) / (MAX_ANGLE - MIN_ANGLE) + (float)SERVO_MIN;
        ...
        MessageBuf[4 * i] = 0x00;
        MessageBuf[1 + 4 * i] = 0x00;
        MessageBuf[2 + 4 * i] = (uint16_t)Value & 0xFF;
        MessageBuf[3 + 4 * i] = (uint16_t)Value >> 8;
    }
    
    while(HAL_I2C_GetState(pca9685_i2c) != HAL_I2C_STATE_READY);
	if(HAL_OK != HAL_I2C_Mem_Write_DMA(pca9685_i2c, PCA9685_ADDRESS, RegisterAddress, I2C_MEMADD_SIZE_8BIT, MessageBuf, 4 * num))
	{
		return PCA9685_ERROR;
	}
...
PCA9685_STATUS PCA9685_Init(I2C_HandleTypeDef *hi2c)
{
    if(MessageBuf != NULL)
    {
        vPortFree(MessageBuf);
    }
    MessageBuf = pvPortMalloc(64);
