...
extern "C" void control_task_main(void *argument)
{
    ...
    while (1)
    {
        ...
        for(int i = 0; i < 12; ++i)
        {
            server_angles[i] = RAD2DEG(inverse_array[i] * target_joint_positions[i] + offset_array[i]) + MAX_ANGLE / 2.0f;
        }
        PCA9685_SetServoAngles(server_angles, 0, 12);
